...
# 
# Task:-
# find the emotions from human sentiments.
...
valid=pd.read_csv("project/val.txt",sep=';',names=['sentiment','emotion'])
...
train.shape,test.shape,valid.shape  # shape function is used to find rows and column
...
Emotion_count=valid.groupby('emotion').count()
plt.bar(Emotion_count.index.values, Emotion_count['sentiment'],color=["orange","black","yellow","red","gray","blue"])
plt.xlabel('emotion')
plt.ylabel('Number of emotion')
plt.show()
...
# for each emotion
for emotion, ax in zip(emotions, axes.flat):
    wordcloud = get_wordcloud(test[test["emotion"]==emotion]['sentiment'])
    ax.imshow(wordcloud)
    ax.title.set_text(emotion)
    ax.title.set_size(70)
...
# ## We got Accuracy is 84.463% which is really good better than multinomial naive bayes.
# # Conclusion:
...
# #####                                + stemming
# #####                                + stopwords
# #### 4. we use algorithms multinomial naive bayes and random forest callsifier we got the accuracy of 84.463% from random forest classifer.
# #### 5. from this we can analyse from the human sentiment or sentences we find emotion of human.
